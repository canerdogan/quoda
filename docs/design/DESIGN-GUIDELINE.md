...
| **T3 Accent** | **`#0A7EA4`** deep teal-blue. **4.63:1 on white** (passes WCAG AA normal text), **4.19:1 on near-black** (AA large/UI; for normal text on dark use `--color-accent-hover` or ≥18px/semibold). One accent, once per viewport. | Monitor teal SaaS saturation annually; it's a token, migratable. Indigo `#5B5BD6` was **rejected** — only 3.8:1, fails AA. Accent's primary use is button background with white text (4.63:1 ✓). |
...
4. WCAG 1.4.3 contrast is a shipping gate. Accent measured **4.63:1 on white / 4.19:1 on near-black** (white-on-accent buttons = 4.63:1, passes AA normal text; accent-as-text on dark passes AA large/UI only — use `--color-accent-hover` for normal text on dark). New tokens must carry measured ratios in their definition comment. Live generator fully keyboard- & screen-reader-operable: `role=form`, `aria-label`, `aria-live=polite` on preview, full Tab sequence input→download, no modal traps.
